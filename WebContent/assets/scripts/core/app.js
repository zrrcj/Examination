...
var App = function() {
	...
	var handleSidebarAndContentHeight = function() {
		...
		if (body.hasClass("page-footer-fixed") === true
			...
			var available_height = $(window).height()
					- $('.footer').outerHeight() - $('.header').outerHeight();
				...
				.on(
						...
						function(e) {
							...
							parent.children('li.open').children(
									'.sub-menu:not(.always-open)').slideUp(200);
	...
	var handleSidebarToggler = function() {
		...
		$('.page-sidebar, .header').on(
				...
				function(e) {
					...
					if ((body.hasClass("page-sidebar-hover-on") && body
						...
						handleSidebarAndContentHeight(); // fix content &
															// sidebar height
						if ($.cookie) {
					...
					handleSidebarAndContentHeight(); // fix content & sidebar
														// height
					runResponsiveHandlers();
				});
		...
		// handle the search query submit on enter press
		$('.header .search-form').on('keypress', 'input.form-control',
				function(e) {
					if (e.which == 13) {
						$('.sidebar-search').submit();
						return false; // <---- Add this line
	...
	var handleTheme = function() {
		...
		$(
				'.layout-option, .header-option, .sidebar-option, .footer-option, .sidebar-pos-option',
	...
	return {
		...
		blockUI : function(options) {
			...
			if (options.iconOnly) {
				html = '<div class="loading-message '
						...
						+ '"><img style="" src="./assets/img/loading-spinner-grey.gif" align=""></div>';
				...
				html = '<div class="loading-message '
						+ (options.boxed ? 'loading-message-boxed' : '')
						+ '"><img style="" src="./assets/img/loading-spinner-grey.gif" align=""><span>&nbsp;&nbsp;'
						+ (options.message ? options.message : 'LOADING...')
						+ '</span></div>';
			}
		...
		startPageLoading : function(message) {
			...
			$('body')
					.append(
							'<div class="page-loading"><img src="assets/img/loading-spinner-grey.gif"/>&nbsp;&nbsp;<span>'
